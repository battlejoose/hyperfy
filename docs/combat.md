...
Live corpse spawn (dying client + observers) steals the existing avatar, reparents it while preserving its world transform, and freezes the mixer immediately — no pose snap or deferred frames. Joiners/replays load a fresh avatar at the recorded `p`/`q` and snap to dead pose.
