...
Assets are downloaded, parsed (GLTF/VRM), and cached by `ClientLoader`. `preload` flag causes download at world join to prevent in-game stalls. All downloads go through `extras/downloadBlob.js`: a streaming fetch that keeps received chunks and resumes from the last byte with an HTTP Range request when Heroku drops the body mid-transfer (`ERR_FAILED` with HTTP 200), instead of restarting from byte 0. It also emits byte-level `file-progress` events which drive the loading bar during the arena download. The arena environment ships as `colosseum-v1.glb` (~4MB), a fully procedural in-house model generated by `scripts/colosseum/build.mjs` (zero third-party assets — regenerate with `node scripts/colosseum/build.mjs`). It uses glTF node instancing (one mesh, many nodes) for arches, columns, seats and sails, WebP textures capped at 1024px (keeps GPU VRAM down so mobile/Heroku clients don’t lose the WebGL context), and ships its own simplified collider meshes (`extras.node = collider`) so PhysX never cooks the visual geometry. The two brazier nodes (`barrizer`, `barrizer_2`) anchor the fire FX and Proximo; the seat rows are built on the exact radii/heights in `arenaCrowd.js`. `glbToNodes` still expands any quantized attributes to float32 for CSM/PhysX and both client and server GLTF loaders register `MeshoptDecoder`. The loading-screen Retry button restarts arena loading in-place (same session/username), busting the arena cache first.
