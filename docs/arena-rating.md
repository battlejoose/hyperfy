...
- Event victory: server broadcasts `brVictory` to everyone when a BR or tournament ends (`pending` → `complete`/`failed`). Client shows a shared victory sheet with winner name, abbreviated wallet, champion pot, and betting settlement (pot, winning bettors + per-share payout, or no-winners / refund note) ([`BattleRoyaleVictory.js`](../src/client/components/BattleRoyaleVictory.js)). Champion payout status still streams `pending` → confirmed tx / Solscan.
