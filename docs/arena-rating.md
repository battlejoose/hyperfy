...
- Android MWA entry payments call `ensureLoopbackNetworkAccess()` first ([`solanaWallet.js`](../src/client/extras/solanaWallet.js)): if Chrome still needs Local Network / “Apps on your device” permission, the user grants it (via a Continue sheet + `fetch('http://localhost')`), then wallet selection opens immediately — no second confirm step.
