...
- Android MWA entry payments ([`solanaWallet.js`](../src/client/extras/solanaWallet.js) / [`PlayerQueueList.js`](../src/client/components/PlayerQueueList.js)): Local Network Access is requested before association (Continue → Allow → Open Wallet). After the tab was backgrounded, Join Battle always asks for a fresh “Open Wallet” tap so Chrome still has a trusted gesture. A prior MWA pubkey in UI state no longer skips MWA into a dead standard-wallet recover path. LNA wait / MWA session / Join pending all time out (~25–50s) so the button unlocks for retry; stale `auth_token` is cleared and re-authorized in-session.
