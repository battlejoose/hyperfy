...
4. UI **bet value** per fighter is the LMSR cost of the **next 1 share** vs the cost of the **first share** on an empty market for that fighter (first share = **100%**, rounded). Favored outcomes rise above 100%; neglected ones can fall below.
...
- Event victory: server broadcasts `brVictory` when a BR or tournament ends. The sheet has two stacked sections: **Champion payout** (loading bar → tx hash) then **Betting** (summary + per-bettor loading bars and Solscan hashes as each `bet_win` confirms) ([`BattleRoyaleVictory.js`](../src/client/components/BattleRoyaleVictory.js)).
